...
export async function processPokeList(pokeList) {
  for (const pokeItem of pokeList) {
    ...
    const favIconSrc = isFavorite(pokeId)
    ...
    const buttonActive =
      '<div class="actions"><button class="pokedex-button active">Zum Pokédex</button></div>';
    const buttonSaved =
      '<div class="actions"><button class="pokedex-button saved" disabled>Im Pokédex</button></div>';
    ...
    el.innerHTML = `
            <div class="params"><span class="id"># ${pokeId}</span> <span class="fav"><img src="${favIconSrc}" alt=""></span></div>
...
async function loadNextBatch() {
  ...
  if (pokeList && pokeList.results && pokeList.results.length > 0) {
    console.log(
      "Load next batch: " + currentOffset + " - " + (currentOffset + limit),
    );
...
elContainer.addEventListener("click", (event) => {
  const favSpan = event.target.closest(".fav");

  if (favSpan) {
    const card = favSpan.closest("#pokemon-list > div");
    if (!card) return;

    const pokemonId = Number(card.dataset.id);
    const isFav = toggleFavorite(pokemonId);

    setStarIcon(favSpan.querySelector("img"), isFav);
    return;
  }
